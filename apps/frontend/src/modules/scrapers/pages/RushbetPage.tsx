...
export const RUSHBET_URL = 'https://www.rushbet.co/?page=sportsbook#filter/football/';
const STORAGE_KEY = 'scraping_rushbet_last_result';
const MAX_URLS = 10;
...
// El deporte se identifica por el hash de la URL de Rushbet: #filter/football/, #filter/tennis/, etc.
const PRESETS: SportPreset[] = [
  { label: 'Fútbol', icon: '⚽', url: RUSHBET_URL, sportKey: 'football' },
  { label: 'Tenis', icon: '🎾', url: 'https://www.rushbet.co/?page=sportsbook#filter/tennis/', sportKey: 'tennis' },
  { label: 'Baloncesto', icon: '🏀', url: 'https://www.rushbet.co/?page=sportsbook#filter/basketball/', sportKey: 'basketball' },
  { label: 'Tenis de Mesa', icon: '🏓', url: 'https://www.rushbet.co/?page=sportsbook#filter/table_tennis/', sportKey: 'table_tennis' },
];

function isRushbetDomain(urlStr: string): boolean {
  try {
    const parsed = new URL(urlStr.trim());
    const host = parsed.hostname.toLowerCase().replace(/^www\./, '');
    return host === 'rushbet.co' && (parsed.protocol === 'http:' || parsed.protocol === 'https:');
  } catch {
    return false;
  }
}
...
// Módulo dedicado de scraping para Rushbet Colombia (www.rushbet.co) — motor Kambi (oferta rsico).
// Multi-deporte: hasta 10 URLs por lote. Las cuotas se obtienen vía fetch directo a la API pública
// de Kambi (sin Playwright, sin proxy), incluyendo partidos prepartido y en vivo.
export const RushbetPage: React.FC = () => {
  const [urls, setUrls] = useState<string[]>([RUSHBET_URL]);
  ...
  const invalidDomainUrls = urls.filter((u) => u.trim() && !isRushbetDomain(u));
    ...
    try {
      const res = await fetch(`${API_BASE}/scrape/custom`, {
        ...
        body: JSON.stringify({
          ...
          useProxy: false, // Rushbet va directo a la API de Kambi — el proxy no aplica
          timeoutMs: 45000,
        }),
      });
      ...
      if (data.matches && data.matches.length > 0) {
        ...
        setError(data.message || data.error || 'No se encontraron cuotas en Rushbet.');
  ...
  // Partidos con 1X2/ganador primero (por si algún partido no trae mercado principal)
  ...
  const filterBtnStyle = (active: boolean): React.CSSProperties => ({
    ...
    color: active ? '#fb7185' : 'var(--text-primary)',
    fontSize: '0.8rem',
    fontWeight: 600,
    cursor: 'pointer',
  });
  ...
  return (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)' }}>🔴 Scraping Rushbet (Multi-Deporte)</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '0.25rem' }}>
          Módulo dedicado para www.rushbet.co con soporte para hasta {MAX_URLS} URLs simultáneas. Fetch directo a la API
          pública de Kambi (oferta rsico) — sin Playwright, sin proxy, prepartido y en vivo. Fútbol: 1X2 y Más/Menos 2.5.
          Tenis, baloncesto y tenis de mesa: ganador del partido.
        </p>
      </div>

      <Card title="1. Configurar URLs por Deporte" subtitle={`Hasta ${MAX_URLS} URLs de rushbet.co (${urls.length} / ${MAX_URLS} configuradas)`}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--radius-md)',
                      border: '1px solid var(--border-color)',
                      background: alreadyAdded ? 'rgba(255,255,255,0.02)' : 'rgba(244,63,94,0.12)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {urls.map((u, idx) => {
              const isInvalid = u.trim().length > 0 && !isRushbetDomain(u);
              return (
                <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                    ...
                    <input
                      id={`rushbet-url-${idx}`}
                      type="text"
                      placeholder="https://www.rushbet.co/?page=sportsbook#filter/football/"
                      value={u}
                  ...
                  {isInvalid && (
                    <span style={{ fontSize: '0.75rem', color: '#f87171', marginLeft: '32px' }}>
                      ⚠️ Dominio inválido: solo se permiten URLs de <strong>rushbet.co</strong>.
                    </span>
                  )}
                </div>
              );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
            {hasAdapter === true && <Badge variant="purple">📡 Network Adapter activo (Kambi API · rsico)</Badge>}
            {hasAdapter === false && <Badge variant="warning">⚠️ Adapter no detectado — reinicia el backend</Badge>}
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              ℹ️ Incluye partidos en vivo: sus cuotas cambian en segundos, vuelve a scrapear para refrescarlas. El deporte se detecta por el hash de la URL (#filter/football/).
            </div>
          </div>
        </div>
      </Card>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 'var(--radius-md)',
          border: 'none',
          background: !canScrape ? 'rgba(244,63,94,0.3)' : 'linear-gradient(90deg, #e11d48, #f97316)',
          color: '#fff',
          fontWeight: 700,
      ...
      >
        {loading ? '⏳ Consultando Kambi API...' : `🚀 Ejecutar Scraping Rushbet (${urls.length} deporte${urls.length !== 1 ? 's' : ''})`}
      </button>
      ...
      {results && matches.length > 0 && (
        <div style={{ marginTop: '1.5rem' }}>
          <Card
            ...
            subtitle={`${results.totalMatches || matches.length} partidos en ${results.durationMs}ms — Fuente: Kambi API${results.oddsCount !== undefined ? ` · ${results.oddsCount} cuotas` : ''}`}
          >
            <div style={{ marginBottom: '1.25rem', display: 'flex', gap: '0.6rem', alignItems: 'center', flexWrap: 'wrap' }}>
              <Badge variant="purple">📡 Kambi API rsico (fetchDirect)</Badge>
              {results.oddsCount !== undefined && <Badge variant="success">{results.oddsCount} cuotas</Badge>}
              <Badge variant="warning">Rushbet</Badge>
