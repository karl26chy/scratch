...
export const BETSSON_URL = 'https://www.betsson.co/apuestas-deportivas/futbol?tab=allLeagues';
const STORAGE_KEY = 'scraping_betsson_last_result';
const MAX_URLS = 10;
...
// El deporte se identifica por la ruta de la URL: /apuestas-deportivas/futbol, /tenis, /baloncesto, /tenis-de-mesa.
const PRESETS: SportPreset[] = [
  { label: 'Fútbol', icon: '⚽', url: BETSSON_URL, sportKey: 'football' },
  { label: 'Tenis', icon: '🎾', url: 'https://www.betsson.co/apuestas-deportivas/tenis', sportKey: 'tennis' },
  { label: 'Baloncesto', icon: '🏀', url: 'https://www.betsson.co/apuestas-deportivas/baloncesto', sportKey: 'basketball' },
  { label: 'Tenis de Mesa', icon: '🏓', url: 'https://www.betsson.co/apuestas-deportivas/tenis-de-mesa', sportKey: 'table_tennis' },
];

function isBetssonDomain(urlStr: string): boolean {
  try {
    const parsed = new URL(urlStr.trim());
    const host = parsed.hostname.toLowerCase().replace(/^www\./, '');
    return host === 'betsson.co' && (parsed.protocol === 'http:' || parsed.protocol === 'https:');
  } catch {
    return false;
  }
}
...
// Módulo dedicado de scraping para Betsson Colombia (www.betsson.co).
// Multi-deporte: hasta 10 URLs por lote. Las cuotas se obtienen vía fetch directo a la API interna de Betsson
// (sin Playwright, sin proxy), incluyendo partidos prepartido y en vivo.
export const BetssonPage: React.FC = () => {
  const [urls, setUrls] = useState<string[]>([BETSSON_URL]);
  ...
  const invalidDomainUrls = urls.filter((u) => u.trim() && !isBetssonDomain(u));
    ...
    try {
      const res = await fetch(`${API_BASE}/scrape/custom`, {
        ...
        body: JSON.stringify({
          ...
          useProxy: false, // Betsson va directo a su API interna — el proxy no aplica
          timeoutMs: 45000,
        }),
      });
      ...
      if (data.matches && data.matches.length > 0) {
        ...
        setError(data.message || data.error || 'No se encontraron cuotas en Betsson.');
  ...
  return (
    <div>
      <div style={{ marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)' }}>🔵 Scraping Betsson (Multi-Deporte)</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '0.25rem' }}>
          Módulo dedicado para www.betsson.co con soporte para hasta {MAX_URLS} URLs simultáneas. Fetch directo a la API
          interna de Betsson — sin Playwright, sin proxy, prepartido y en vivo. Fútbol: 1X2, Más/Menos 2.5 y Ambos marcan.
          Tenis, baloncesto y tenis de mesa: ganador del partido.
        </p>
      </div>

      <Card title="1. Configurar URLs por Deporte" subtitle={`Hasta ${MAX_URLS} URLs de betsson.co (${urls.length} / ${MAX_URLS} configuradas)`}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--radius-md)',
                      border: '1px solid var(--border-color)',
                      background: alreadyAdded ? 'rgba(255,255,255,0.02)' : 'rgba(59,130,246,0.12)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {urls.map((u, idx) => {
              const isInvalid = u.trim().length > 0 && !isBetssonDomain(u);
              return (
                <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                    ...
                    <input
                      id={`betsson-url-${idx}`}
                      type="text"
                      placeholder="https://www.betsson.co/apuestas-deportivas/futbol"
                      value={u}
                  ...
                  {isInvalid && (
                    <span style={{ fontSize: '0.75rem', color: '#f87171', marginLeft: '32px' }}>
                      ⚠️ Dominio inválido: solo se permiten URLs de <strong>betsson.co</strong>.
                    </span>
                  )}
                </div>
              );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
            {hasAdapter === true && <Badge variant="purple">📡 Network Adapter activo (API interna de Betsson)</Badge>}
            {hasAdapter === false && <Badge variant="warning">⚠️ Adapter no detectado — reinicia el backend</Badge>}
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              ℹ️ Incluye partidos en vivo: sus cuotas cambian en segundos, vuelve a scrapear para refrescarlas. El deporte se detecta por la ruta de la URL (/apuestas-deportivas/futbol).
            </div>
          </div>
        </div>
      </Card>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 'var(--radius-md)',
          border: 'none',
          background: !canScrape ? 'rgba(59,130,246,0.3)' : 'linear-gradient(90deg, #2563eb, #06b6d4)',
          color: '#fff',
          fontWeight: 700,
      ...
      >
        {loading ? '⏳ Consultando API de Betsson...' : `🚀 Ejecutar Scraping Betsson (${urls.length} deporte${urls.length !== 1 ? 's' : ''})`}
      </button>
      ...
      {results && matches.length > 0 && (
        <div style={{ marginTop: '1.5rem' }}>
          <Card
            ...
            subtitle={`${results.totalMatches || matches.length} partidos en ${results.durationMs}ms — Fuente: API Betsson${results.oddsCount !== undefined ? ` · ${results.oddsCount} cuotas` : ''}`}
          >
            <div style={{ marginBottom: '1.25rem', display: 'flex', gap: '0.6rem', alignItems: 'center', flexWrap: 'wrap' }}>
              <Badge variant="purple">📡 API Betsson (fetchDirect)</Badge>
              {results.oddsCount !== undefined && <Badge variant="success">{results.oddsCount} cuotas</Badge>}
              <Badge variant="warning">Betsson</Badge>
